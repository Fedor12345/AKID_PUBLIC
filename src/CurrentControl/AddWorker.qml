...
//                                            }
        ...
        Button {
            ...
            onClicked: {
                ...
                ///ДАТА РАСКОМЕНТИРОВАТЬ if (nw_birthday.ready) data_arr["BIRTH_DATE"] = nw_birthday.date_val
                ...
                if (nw_personalNumber.length > 0) data_arr["PERSON_NUMBER"] = nw_personalNumber.text
                if (nw_tld.text.length > 0)       data_arr["ID_TLD"]    = nw_tld.text

                data_arr["STATUS_CODE"] = modeles.model_adm_status.getId(nw_statusCode.currentIndex) //nw_statusCode.currentIndex + 1
                ...
                if(nw_staffType.currentIndex==0) {
                    data_arr["ID_DEPARTMENT_NPP"] = modeles.model_adm_department_nnp.getId(nw_department_npp.currentIndex)
                }
                else if(nw_staffType.currentIndex==1) {
                    data_arr["ID_ORGANIZATION"] = modeles.model_adm_organisation_dep.getId(nw_department.currentIndex)
                }

                //data_arr["ID_ASSIGNEMENT"] = nw_assignment.currentText
...
////                }
                ...
                //if (nw_pass_serial.text.length > 0) data_arr["PASSPORT_SERIAL"] = nw_pass_serial.text
